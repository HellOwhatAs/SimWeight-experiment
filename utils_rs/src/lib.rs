use std::{cmp::Reverse, collections::{BinaryHeap, HashMap, HashSet}, sync::Mutex};
use pyo3::prelude::*;
use rayon::prelude::*;
...
pub struct DiGraph {
    ...
    cache: Mutex<Vec<Vec<Option<f64>>>>
}
...
impl DiGraph {
    ...
    pub fn new(n: usize, edges: Vec<(usize, usize)>, weight: Option<Vec<f64>>, pos: Option<Vec<(f64, f64)>>) -> Self {
        ...
        let cache = Mutex::new(vec![vec![None; n]; n]);
        Self { n, edges, adjlist, radjlist, weight, pos, cache }
    }
    ...
    pub fn bidirectional_astar(&self, u: usize, v: usize, k: usize, weight: Option<Vec<f64>>) -> Vec<Vec<usize>> {
        ...
        let h = |u: usize, v: usize| -> f64 {
            *self.cache.lock().unwrap()[u][v].get_or_insert_with(|| pos[u].haversine_distance(&pos[v]))
        };
